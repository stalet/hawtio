...
/// <reference path="../../core/js/coreGlobals.ts"/>
module ControllerHelpers {
  ...
  export function bindModelToSearchParam($scope, $location, modelName:string, paramName:string, initialValue?:any, to?: (value:any) => any, from?: (value:any) => any) {
    ...
    var toConverter = to || Helpers.doNothing
    var fromConverter = from || Helpers.doNothing;
    ...
    ObjectHelpers.pathSet($scope, modelName, value);
   ...
   */
  export function reloadWhenParametersChange($route:any, $scope:ng.IScope, $location?:ng.ILocationService, parameters = ["nid"]) {
    if (!$location) {
      $location = Core.injector.get('$location');
    }
